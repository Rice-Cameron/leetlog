  ...
  switch (mode) {
    ...
    case '3': // Test
      return {
        mode: 'test',
        url: process.env.DATABASE_URL_TEST || '',
        neonProjectId: undefined, // Test database doesn't need Neon metadata
        neonBranch: 'test-leetlog-db',
      }
      
    default:
      console.warn(`⚠️  Unknown DATABASE_MODE: ${mode}, defaulting to development`)
      return {
        mode: 'development',
        url: process.env.DATABASE_URL_DEV || '',
        neonProjectId: process.env.NEON_PROJECT_ID_DEV,
...
export function getDatabaseUrl(): string {
  ...
  if (config.mode === 'test' && config.url.includes('ep-dark-surf')) {
    throw new Error('🚨 SAFETY VIOLATION: Test mode attempted to use production database!')
  }
  
  console.log(`📊 Active Database: ${config.mode.toUpperCase()} (${config.url.substring(0, 50)}...)`)
...
export function validateDatabaseConfig(): void {
  ...
  if (!config.url) {
    throw new Error(`Missing database URL for ${config.mode} mode. Check your environment variables.`)
  }
  
  // Safety verification
  if (config.mode === 'test') {
    if (config.url.includes('ep-dark-surf')) {
      throw new Error('🚨 CRITICAL SAFETY ERROR: Test mode is using production database URL!')
    }
    console.log(`✅ Safety Check: Test mode using isolated database (ep-restless-cloud)`)
  }
  
  if (config.mode === 'production') {
    if (!config.url.includes('ep-dark-surf')) {
      console.warn(`⚠️  Warning: Production mode not using expected production branch`)
    }
